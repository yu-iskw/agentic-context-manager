...
3. Tenant/scope authorization is applied before retrieval ranking.
...
7. MCP/REST adapters remain thin; lifecycle logic belongs in shared services/packages.
8. Recalled/stored content is untrusted historical data and never gains instruction priority.
9. Optional graph/cache/vector systems are introduced only after benchmarks justify them.
10. Docker Compose is the normative integration-test environment and CI must run the same integration command as local development.
...
- first-party application SDK middleware;
...
- **Unit tests:** Vitest.
- **Integration tests:** Docker Compose plus dependency-free Node test runner.
...
packages/core/          transport-independent ranking/packing logic
...
Before a PR is considered ready, run the applicable local gates. Changes touching the lifecycle, database, API, worker, MCP, Compose, or provider behavior should run **both** unit tests and `pnpm test:integration:compose`.
...
2. constrain workspace/task/session scope before ranking;
...
- plugin conformance;
- compaction fallback once implemented.
